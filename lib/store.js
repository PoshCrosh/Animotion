...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'RESET':
      return { ...FRESH_STATE, dark: state.dark }; // keep dark mode pref
...
export function AppProvider({ children }) {
  ...
  // This is the single source of truth — Settings just dispatches SET_DARK
  useEffect(() => {
    if (typeof document === 'undefined') return;
    document.documentElement.classList.toggle('dark', !!state.dark);
    localStorage.setItem('animotion_dark', JSON.stringify(!!state.dark));
  }, [state.dark]);

  // Load dark preference on first mount
  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('animotion_dark') ?? 'false');
      if (saved) dispatch({ type: 'SET_DARK', v: true });
    } catch {}
  }, []); // eslint-disable-line
