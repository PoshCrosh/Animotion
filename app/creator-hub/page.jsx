...
const CATS = ['All','2D','3D','VFX','Motion Graphics','General'];
const CAT_COLORS = { '2D':'#5B9CF6','3D':'#9B7FEA','VFX':'#FF5757','Motion Graphics':'#FFB347','General':'#52C97C' };
    ...
    <AppShell user={user}>
      <div className="page">
        ...
        <div className="wii-card p-5 mb-6 flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl" style={{ background:'linear-gradient(135deg,#9B7FEA,#7C5FD4)', boxShadow:'0 4px 16px rgba(155,127,234,0.4)' }}>🎬</div>
          <div>
            <h1 className="font-black text-2xl" style={{ color:'#1E3A4A' }}>Creator Hub</h1>
            <p className="text-[13px] font-semibold" style={{ color:'#7A9BAA' }}>Learn from the best animation educators</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {filtered.map(creator => (
            <div key={creator.name} className="wii-card p-5 flex flex-col">
              <div className="flex items-center gap-4 mb-3">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl flex-shrink-0"
                  style={{ background:`${CAT_COLORS[creator.category]??'#009AC7'}18`, border:`1.5px solid ${CAT_COLORS[creator.category]??'#009AC7'}33` }}>
                  🎥
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-black text-[14px] mb-0.5" style={{ color:'#1E3A4A' }}>{creator.name}</h3>
                  <span className="text-[10px] font-black px-2 py-0.5 rounded-full"
                    style={{ background:`${CAT_COLORS[creator.category]??'#009AC7'}15`, color:CAT_COLORS[creator.category]??'#009AC7' }}>
                    {creator.category}
                  </span>
                </div>
              </div>
              <p className="text-[12px] font-semibold leading-relaxed flex-1 mb-4" style={{ color:'#7A9BAA' }}>{creator.desc}</p>
