...
export default function ProgressPage() {
    ...
    return (
      <AppShell user={user}>
        <ActiveLesson
          ...
          onComplete={(l) => { completeLesson(l.id, l.xp); setActiveLesson(null); setActiveWorld(null); }}
  ...
  // Build node list with world context
                    ...
                    {ri > 0 && (
                      <div className="flex justify-center my-1">
                        <svg width="32" height="28" viewBox="0 0 32 28" fill="none" aria-hidden="true">
                          <path
                            d={`M16 0 C${reversed ? '2' : '30'} 0 ${reversed ? '2' : '30'} 28 16 28`}
                            stroke="#E5E7EB" strokeWidth="2.5" strokeDasharray="5 3"
                          ...
                          <div key={node.id} className="flex items-center gap-3">
                            ...
                            >
                              ...
                              >
                                ...
                                <span
                                  className="text-[8px] font-black text-center leading-tight mt-0.5 px-1"
                                  style={{
                                    color: node.isDone ? 'white' : node.isLocked ? '#9CA3AF' : node.world.color,
                                    maxWidth: 56,
                                  }}
                                >
                            ...
                            {ni < display.length - 1 && (
                              <svg width="36" height="6" viewBox="0 0 36 6" fill="none" aria-hidden="true">
                                <line x1="0" y1="3" x2="36" y2="3"
                                  ...
                                  strokeWidth="2.5"
                                  strokeDasharray={node.isDone ? 'none' : '5 3'}
          ...
          <div className="space-y-4">
            ...
            {hovered ? (
              <div className="card p-5 animate-fade-up">
                ...
                <div className="flex gap-2 flex-wrap mb-4">
                  <span className="tag text-[10px]" style={{ background:`${hovered.world.color}22`, color:hovered.world.color }}>
                    {hovered.diff}
                  </span>
            ...
            <div className="card p-5">
              ...
              <div className="space-y-3">
                {WORLDS.map((w) => {
                  const done = w.lessons.filter((l) => completedLessons.includes(l.id)).length;
                  return (
                    <div key={w.id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between mb-1">
                          ...
                          <span className="text-[10px] font-black text-gray-400 ml-1">{done}/{w.lessons.length}</span>
                        </div>
                        <div className="h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                          <div className="h-full rounded-full transition-[width] duration-700"
                            style={{ width:`${(done/w.lessons.length)*100}%`, background:w.color }} />
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div className="card p-5">
              ...
              <div className="h-2.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-[width] duration-700"
                  style={{ width:`${(totalDone/ALL_LESSONS.length)*100}%`, background:'linear-gradient(90deg,#5B9CF6,#9B7FEA)' }} />
              </div>
