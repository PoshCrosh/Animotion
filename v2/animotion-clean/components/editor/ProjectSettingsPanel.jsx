  ...
  return (
    <div className="card p-5 animate-fade-up">
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={onClose} className="text-xl text-gray-400 hover:text-gray-600 w-7 h-7 flex items-center justify-center">×</button>
      </div>
          ...
          <div className="flex gap-2">
            {[12, 24, 30].map((f) => (
              <button key={f} onClick={() => ed.setFps(f)}
                className={`flex-1 py-2 rounded-xl border text-xs font-black transition-all ${state.fps === f ? 'border-primary bg-primary-light text-primary-dark' : 'border-gray-200 dark:border-gray-700 hover:border-primary text-gray-600'}`}>
                {f} fps
              </button>
            ))}
          </div>
