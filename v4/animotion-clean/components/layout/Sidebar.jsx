...
const NAV = [
  { section: 'Learning', items: [
    ...
    { href: '/roadmap',      label: 'Smart Roadmap',    icon: '🧭', badge: 'AI' },
    { href: '/creator-hub',  label: 'Creator Hub',      icon: '🎬', badge: 'NEW' },
  ]},
  { section: 'Practice', items: [
    { href: '/editor',       label: 'Animation Editor', icon: '✏️' },
    { href: '/coach',        label: 'AI Coach',         icon: '🤖', badge: 'AI' },
    { href: '/challenges',   label: 'Weekly Challenge', icon: '⚔️', badge: 'NEW' },
  ]},
...
export default function Sidebar({ user }) {
  ...
  const displayName = user?.user_metadata?.name ?? user?.user_metadata?.full_name ?? user?.email?.split('@')[0] ?? 'Animator';
                ...
                <Link key={item.href} href={item.href}
                  ...
                  {item.badge && (
                    <span className={`text-[9px] font-black px-1.5 py-0.5 rounded-full text-white ${
                      item.badge === 'AI' ? 'bg-pink' : 'bg-green-500'
                    }`}>
                      {item.badge}
                    </span>
                  )}
